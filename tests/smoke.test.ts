...
describe('Task 5 flagship home styling', () => {
  ...
  it('builds two stable photographic collection worlds with restrained accents', () => {
    ...
    expect(globalsCss).toMatch(/\.collection-world-media\s*\{[^}]*aspect-ratio:\s*16\s*\/\s*10;[^}]*overflow:\s*hidden;[^}]*border-radius:\s*(?:[0-8](?:px)?|var\([^)]*\))/)
  ...
  it('gives collection sections stable responsive product grids and truthful empty bands', () => {
    ...
    expect(globalsCss).not.toMatch(/@media \(max-width:\s*1024px\)[\s\S]*?\.collection-world-grid\s*\{[^}]*display:\s*flex;[^}]*overflow-x:\s*auto/)
    expect(globalsCss).not.toMatch(/@media \(max-width:\s*1024px\)[\s\S]*?\.collection-world-panel\s*\{[^}]*flex:\s*0 0/)
    ...
    expect(globalsCss).toMatch(/@media \(max-width:\s*560px\)[\s\S]*?\.collection-world-grid\s*\{[^}]*gap:\s*0\.75rem/)
