...
const PAYMENT: Record<string, string> = { demo: 'Simulación', sinpe: 'SINPE Móvil', link: 'Link de pago', cash: 'Efectivo' }
...
export default function OrderPresentation({ order, view }: { order: CommerceOrder; view: 'confirmation' | 'tracking' }) {
  if (view === 'confirmation') {
    return (
      <div className="order-page container">
        <div className="confirmation-lead"><span className="confirmation-icon"><Check aria-hidden="true" size={28} /></span><p className="section-label">FYTHER / CONFIRMACIÓN</p><h1 className="display">Pedido confirmado.</h1><p>Tu número de pedido es <strong>{order.orderNumber}</strong>.</p>{order.demo && <p className="demo-order-note">Esta es una simulación. No se realizó ningún cobro ni cambio de inventario.</p>}<Link className="button button-accent" href={`/tracking/${order.id}`}>Seguir pedido</Link></div>
  ...
  return (
    <div className="order-page container">
      ...
      <header className="tracking-head"><div><p className="section-label">FYTHER / TRACKING</p><h1 className="display">En movimiento.</h1><p>Pedido {order.orderNumber}</p></div><strong>{STATUS[order.status] ?? order.status}</strong></header>
      {order.demo && <p className="demo-order-note">Seguimiento de demostración. No corresponde a un envío real.</p>}
